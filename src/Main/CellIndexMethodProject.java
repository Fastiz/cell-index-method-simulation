...
public class CellIndexMethodProject {
    ...
    private void execute(ArrayList<Particle> particles, float searchRadius, float mapSideSize, boolean isPeriodic, float maxRadius, int numberOfCells) {
        ...
        CellMap cellMap = new CellMap(particles, searchRadius, numberOfCells, mapSideSize, isPeriodic, maxRadius);
        ...
        try {
            ...
            for (Particle particle : particles) {
                if (particle.getPos().getX() > ( 2*mapSideSize) / 5 && particle.getPos().getX() < (4 * mapSideSize) / 8 && particle.getPos().getY() > selectedParticle.getPos().getY())
                    if (flag && particle.getPos().getX() < selectedParticle.getPos().getX())
